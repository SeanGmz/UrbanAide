...
class EventsPage(Frame):
    def __init__(self, parent, controller):
        ...
        sortFrame = customtkinter.CTkOptionMenu(titleFrame, values = sortOptions, command=self.sort_events, width=120, height=30, font=("Krub", 16), dropdown_font=("Krub", 16), dropdown_fg_color="#dadada", fg_color="#dadada", button_color="#dadada", text_color ="#000000", dropdown_text_color="#000000", button_hover_color="#d4d4d4", dropdown_hover_color="#d4d4d4")
        ...
        self.ongoingFrame = Frame(eventFrame, borderwidth=5, relief="groove")
        self.ongoingFrame.pack(side="top", fill="x", expand=True)
        Label(self.ongoingFrame, text="Ongoing Events", font=("Krub", 15)).pack(side="top", anchor='w', padx=10, pady=10)
        
        # Container for Ongoing events
        ogEventframe = Frame(self.ongoingFrame, borderwidth=5, relief="groove", height=400, width=100)
        ...
        self.upcomingFrame = Frame(eventFrame, borderwidth=5, relief="groove")
        self.upcomingFrame.pack(side="top", fill="x", expand=True)
        Label(self.upcomingFrame, text="Upcoming Events", font=("Krub", 15)).pack(side="top", anchor="w", padx=10, pady=10)
        
        upEventframe = Frame(self.upcomingFrame, borderwidth=5, relief="groove", height = 400, width=100)
        ...
        self.endedFrame = Frame(eventFrame, borderwidth=5, relief="groove")
        self.endedFrame.pack(side="top", fill="x", expand=True)
        Label(self.endedFrame, text="Ended Events", font=("Krub", 15)).pack(side="top", anchor="w", padx=10, pady=10)
        
        enEventframe = Frame(self.endedFrame, borderwidth=5, relief="groove", height = 400, width=100)
